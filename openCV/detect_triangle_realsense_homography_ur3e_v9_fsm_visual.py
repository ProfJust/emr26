...
- Pick-Sequenz als einfacher Zustandsautomat realisiert.
- Visualisierung von Greiferfinger, Greiferachse und Weltkoordinatensystem ergaenzt.
...
# Visualisierung des Robotiq-2F-Greifers im Bild, Laengen in Pixeln
GRIPPER_AXIS_HALF_LENGTH_PX = 55
GRIPPER_FINGER_LENGTH_PX = 35
GRIPPER_FINGER_OFFSET_PX = 30

# Darstellung des Weltkoordinatensystems im Kamerabild
WORLD_AXIS_ORIGIN_MM = (0.0, 0.0)
...
def draw_gripper_symbol(img: np.ndarray, center: tuple[int, int], edge_angle_deg: float,
                        label: str = "Greifer"):
    """Zeichnet eine schematische Draufsicht des Robotiq-2F-Greifers.

    Greiferachse: parallel zur laengsten Dreieckskante.
    Schliessrichtung/Fingerbewegung: senkrecht dazu.
    """
    cx, cy = int(center[0]), int(center[1])
    a = math.radians(edge_angle_deg)
    axis = np.array([math.cos(a), math.sin(a)], dtype=float)
    close = np.array([-math.sin(a), math.cos(a)], dtype=float)

    p_left = np.array([cx, cy], dtype=float) - GRIPPER_AXIS_HALF_LENGTH_PX * axis
    p_right = np.array([cx, cy], dtype=float) + GRIPPER_AXIS_HALF_LENGTH_PX * axis

    # Greiferachse
    cv2.line(img, tuple(np.round(p_left).astype(int)), tuple(np.round(p_right).astype(int)), (0, 255, 255), 2)

    # Zwei Finger als kurze parallele Linien
    for sign in (-1, 1):
        base = np.array([cx, cy], dtype=float) + sign * GRIPPER_FINGER_OFFSET_PX * axis
        f1 = base - 0.5 * GRIPPER_FINGER_LENGTH_PX * close
        f2 = base + 0.5 * GRIPPER_FINGER_LENGTH_PX * close
        cv2.line(img, tuple(np.round(f1).astype(int)), tuple(np.round(f2).astype(int)), (0, 200, 255), 4)

    # Schliessrichtung als Doppelpfeil
    p1 = np.array([cx, cy], dtype=float) - 35 * close
    p2 = np.array([cx, cy], dtype=float) + 35 * close
    cv2.arrowedLine(img, tuple(np.round(p1).astype(int)), tuple(np.round(p2).astype(int)), (0, 255, 0), 2, tipLength=0.25)
    cv2.arrowedLine(img, tuple(np.round(p2).astype(int)), tuple(np.round(p1).astype(int)), (0, 255, 0), 2, tipLength=0.25)

    cv2.circle(img, (cx, cy), 5, (0, 0, 255), -1)
    cv2.putText(img, label, (cx + 10, cy + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 255, 255), 1, cv2.LINE_AA)
...
class HomographyModel:
    ...
    def world_to_pixel(self, x_mm: float, y_mm: float) -> Optional[tuple[int, int]]:
        """Weltkoordinaten -> Pixel. Nur verfuegbar, wenn eine Homographie aktiv ist."""
        if self.H is None:
            return None
        Hinv = np.linalg.inv(self.H)
        p = np.array([[[float(x_mm), float(y_mm)]]], dtype=np.float32)
        uv = cv2.perspectiveTransform(p, Hinv)[0, 0]
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # Pick-Zustandsautomat
        self.pick_state = PickState.IDLE
        self.pick_state_busy = False
        self.pick_timer = QtCore.QTimer(self)
        self.pick_timer.timeout.connect(self.run_pick_state_machine)
        self.pick_timer.start(50)
    ...
    def _build_robot_tab(self):
        ...
        self.lbl_pick_state = QtWidgets.QLabel("Pick-Zustand: IDLE")
        self.lbl_pick_state.setStyleSheet("font-weight: bold; padding: 6px; background-color: #e8e8e8;")
        layout.addWidget(self.lbl_pick_state)
        ...
        self.btn_abort_pick = QtWidgets.QPushButton("Pick stoppen")
        self.btn_home.setToolTip("Faehrt die eingestellte Home-Gelenkposition mit moveJ an.")
        self.btn_abort_pick.setToolTip("Stoppt den Zustandsautomaten und sendet stopL/stopJ.")
    ...
    def update_frame(self):
        ...
        self.draw_world_coordinate_system(overview)
        if result is not None:
            draw_gripper_symbol(overview, (ROI_X + result.gx_roi, ROI_Y + result.gy_roi), result.edge_angle_deg, "Greifer")
            draw_gripper_symbol(eval_img, (result.gx_roi, result.gy_roi), result.edge_angle_deg, "Greifer")
    ...
    def draw_world_coordinate_system(self, img: np.ndarray):
        """Zeichnet X/Y-Weltachsen ins Kamerabild, falls Homographie aktiv ist."""
        if not self.homography.is_valid():
            cv2.putText(img, "Welt-KS: keine Homographie", (12, img.shape[0] - 18),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 255), 2, cv2.LINE_AA)
            return
        origin = self.homography.world_to_pixel(*WORLD_AXIS_ORIGIN_MM)
        x_end = self.homography.world_to_pixel(WORLD_AXIS_ORIGIN_MM[0] + WORLD_AXIS_LENGTH_MM, WORLD_AXIS_ORIGIN_MM[1])
        y_end = self.homography.world_to_pixel(WORLD_AXIS_ORIGIN_MM[0], WORLD_AXIS_ORIGIN_MM[1] + WORLD_AXIS_LENGTH_MM)
        if origin is None or x_end is None or y_end is None:
            return
        ox, oy = origin
        cv2.circle(img, origin, 6, (255, 255, 255), -1)
        cv2.arrowedLine(img, origin, x_end, (0, 0, 255), 3, tipLength=0.18)
        cv2.arrowedLine(img, origin, y_end, (0, 255, 0), 3, tipLength=0.18)
        cv2.putText(img, "X", (x_end[0] + 6, x_end[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.putText(img, "Y", (y_end[0] + 6, y_end[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2, cv2.LINE_AA)
        cv2.putText(img, "Welt-KS", (ox + 8, oy - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 2, cv2.LINE_AA)
    ...
    def set_pick_state(self, state: PickState):
        self.pick_state = state
        if hasattr(self, "lbl_pick_state"):
            self.lbl_pick_state.setText(f"Pick-Zustand: {state.name}")
            if state == PickState.IDLE:
                self.lbl_pick_state.setStyleSheet("font-weight: bold; padding: 6px; background-color: #e8e8e8;")
            elif state == PickState.ERROR:
                self.lbl_pick_state.setStyleSheet("font-weight: bold; padding: 6px; background-color: #ffb0b0;")
            elif state == PickState.FINISHED:
                self.lbl_pick_state.setStyleSheet("font-weight: bold; padding: 6px; background-color: #b8ffb8;")
            else:
                self.lbl_pick_state.setStyleSheet("font-weight: bold; padding: 6px; background-color: #fff2a8;")
        self.log_robot(f"Pick-Zustand -> {state.name}")

    def execute_pick_sequence(self):
        """Startet die Pick-Sequenz als Zustandsautomat.

        Der Zielpunkt wird im Zustand LOCK_TARGET exakt einmal aus der aktuellen
        Bildverarbeitung uebernommen. Danach bleibt die Zielpose konstant, auch
        wenn der Roboter die Kamera verdeckt.
        """
        if not self.require_robot_enabled():
            return
        if self.pick_state not in (PickState.IDLE, PickState.FINISHED, PickState.ERROR):
            self.log_robot(f"Pick-Sequenz laeuft bereits: {self.pick_state.name}")
            return
        self.locked_target_pose = None
        self.locked_approach_pose = None
        self.set_pick_state(PickState.LOCK_TARGET)

    def abort_pick_sequence(self):
        try:
            if self.rtde_c is not None:
                try:
                    self.rtde_c.stopL(1.0)
            ...
            self.pick_state_busy = False
            self.set_pick_state(PickState.IDLE)
            self.log_robot("Pick-Sequenz gestoppt.")

    def run_pick_state_machine(self):
        """Einfacher sequenzieller Zustandsautomat fuer Pick.

        Die UR_RTDE-Bewegungsbefehle sind blockierend. Deshalb wird pro Timer-
        Durchlauf genau ein Zustand ausgefuehrt. Nach Ende eines Befehls springt
        der Automat in den naechsten Zustand.
        """
        if self.pick_state == PickState.IDLE or self.pick_state_busy:
            return
        if self.pick_state == PickState.FINISHED:
            self.set_pick_state(PickState.IDLE)
            return
        if self.pick_state == PickState.ERROR:
            return
        if not self.require_robot_enabled():
            self.set_pick_state(PickState.ERROR)
            return

        self.pick_state_busy = True
        try:
            speed = float(self.spin_speed.value())
            accel = float(self.spin_accel.value())

            if self.pick_state == PickState.LOCK_TARGET:
                locked = self.lock_current_target_pose("Pick-Zielpose")
                if locked is None:
                    self.set_pick_state(PickState.ERROR)
                    return
                pose, approach = locked
                if not self.validate_pose_with_ik(approach, "FSM-Anfahrpose"):
                    self.set_pick_state(PickState.ERROR)
                    return
                if not self.validate_pose_with_ik(pose, "FSM-Greifpose"):
                    self.set_pick_state(PickState.ERROR)
                    return
                self.set_pick_state(PickState.OPEN_GRIPPER)

            elif self.pick_state == PickState.OPEN_GRIPPER:
                if self.gripper is not None:
                    self.move_gripper(0, speed=80, force=20)
                else:
                    self.log_robot("Kein Gripper verbunden: OPEN_GRIPPER wird uebersprungen.")
                self.set_pick_state(PickState.MOVE_PREGRASP)

            elif self.pick_state == PickState.MOVE_PREGRASP:
                self.log_robot("FSM: Fahre Anfahrpose ueber Greifpunkt.")
                self.rtde_c.moveL(self.locked_approach_pose, speed, accel)
                self.set_pick_state(PickState.MOVE_GRASP)

            elif self.pick_state == PickState.MOVE_GRASP:
                self.log_robot("FSM: Senke auf Greifpose ab.")
                self.rtde_c.moveL(self.locked_target_pose, speed, accel)
                self.set_pick_state(PickState.CLOSE_GRIPPER)

            elif self.pick_state == PickState.CLOSE_GRIPPER:
                if self.gripper is not None:
                    self.move_gripper(180, speed=64, force=50)
                else:
                    self.log_robot("Kein Gripper verbunden: CLOSE_GRIPPER wird uebersprungen.")
                time.sleep(0.2)
                self.set_pick_state(PickState.LIFT_OBJECT)

            elif self.pick_state == PickState.LIFT_OBJECT:
                self.log_robot("FSM: Hebe Objekt wieder auf Anfahrhoehe.")
                self.rtde_c.moveL(self.locked_approach_pose, speed, accel)
                self.set_pick_state(PickState.FINISHED)
            ...
            try:
                ...
                pass
            self.log_robot(f"Pick-FSM Fehler in Zustand {self.pick_state.name}: {e}")
            self.set_pick_state(PickState.ERROR)
        finally:
            self.pick_state_busy = False
